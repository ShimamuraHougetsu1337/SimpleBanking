...
- **Concurrency Integration Tests** (Module 7.2): Added `backend/test/concurrency.spec.ts` to simulate parallel transaction processing using `Promise.all` on `TransactionsService.withdraw` and `transfer`. Covers happy path withdrawals, race condition checks (over-limit failures), and cross-transfer deadlock verification.
- **Test Database Configuration**: Added `DATABASE_URL_TEST` environment variable to `.env.example` for isolated test database setup.

### Changed
- **Deadlock Resolution in Transactions**: Fixed a PostgreSQL deadlock bug where concurrent transactions on the same account crashed due to FK share lock upgrades. Pre-locked accounts via `lockAccounts` before saving the transaction log in `withdraw` and `transfer` methods of `TransactionsService`.
